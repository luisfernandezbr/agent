...
func TestConfigOAuth2Auth(t *testing.T) {
	...
	assert.NoError(cfg.Parse([]byte(pjson.Stringify(map[string]interface{}{"oauth2_auth": oauth2Auth{auth{"url"}, "a", ps.Pointer("b"), nil}}))))
...
func TestConfigOAuth2AuthWithScopes(t *testing.T) {
	...
	cfg := NewConfig(nil)
	assert.Nil(cfg.OAuth2Auth)
	assert.NoError(cfg.Parse([]byte(pjson.Stringify(map[string]interface{}{"oauth2_auth": oauth2Auth{auth{"url"}, "a", ps.Pointer("b"), ps.Pointer("scope")}}))))
	assert.NotNil(cfg.OAuth2Auth)
	assert.Equal("url", cfg.OAuth2Auth.URL)
	assert.Equal("a", cfg.OAuth2Auth.AccessToken)
	assert.Equal("b", *cfg.OAuth2Auth.RefreshToken)
